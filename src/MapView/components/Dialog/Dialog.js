...
const Dialog = ({ locationDetails, onClick, onDismiss, ariaLabel }) => (
  <ReachDialog onDismiss={onDismiss} aria-label={ariaLabel}>
    <button className={styles["dialog__button"]} aria-label="close dialog" onClick={onClick}>
      <XCircleIcon />
    </button>

    <div className={styles["dialog__image"]}>
      <img
        src={locationDetails?.thumbnail?.source || `${process.env.PUBLIC_URL}/placeholder.png`}
